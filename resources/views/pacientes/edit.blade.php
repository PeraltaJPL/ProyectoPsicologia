    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10 bg-light p-4">
                ...
                <form action="{{ route('pacientes.update', $patient->patientId) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="controlNumber" id="controlNumber" class="form-control" value="{{ $patient->controlNumber }}" required oninput="calculateSchoolCycle()">
                    </div>
                    <div class="form-group">
                        <label for="career">Carrera:</label>
                        <input type="text" name="career" id="career" class="form-control" value="{{ $patient->career }}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="schoolCycle">Ciclo Escolar:</label>
                        <input type="text" name="schoolCycle" id="schoolCycle" class="form-control" value="{{ $patient->schoolCycle }}" readonly>
                    </div>
                    <button type="submit" class="btn btn-warning mt-3">Actualizar</button>
                    <a href="{{ route('pacientes.index', ['career' => $patient->career]) }}" class="btn btn-secondary mt-3">Cancelar</a>
                </form>
            </div>
        </div>
    </div>
